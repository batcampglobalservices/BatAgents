...
export default function WalletConnectButton({ variant = "panel" }: WalletConnectButtonProps) {
  ...
  if (variant === "compact") {
    return (
      <div className="flex items-center gap-2">
        {isConnected ? (
          <>
            <div className="hidden rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-medium text-slate-200 md:inline-flex">
          ...
          <button
            ...
            onClick={async () => {
              setLocalError(null);
              try {
                await connectAsync({ connector: availableConnectors[0] });
              } catch (connectError) {
                setLocalError(
            ...
            className="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-2 text-xs font-semibold text-cyan-100 transition hover:border-cyan-400/30 hover:bg-cyan-400/15"
          >
            Connect wallet
          </button>
        ) : (
          <div className="rounded-full border border-amber-400/20 bg-amber-400/10 px-3 py-2 text-xs font-medium text-amber-100">
            Install wallet
          </div>
        )}
      </div>
